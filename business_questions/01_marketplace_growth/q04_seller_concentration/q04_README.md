...
**Seller-attributed GMV definition:**

For seller-level concentration, GMV was calculated at order-item level as:

`price + freight_value`

This represents seller-attributed item revenue, not payment-level GMV. This approach was used because seller attribution exists in `order_items`, while payment records exist at order level and can create many-to-many duplication when joined directly to item rows. Using item-level `price + freight_value` preserves seller attribution and avoids payment-row inflation.
...
3. aggregate seller-attributed GMV as sum(price + freight_value)
...
> - cumulative seller-attributed GMV share
> - Gini coefficient
