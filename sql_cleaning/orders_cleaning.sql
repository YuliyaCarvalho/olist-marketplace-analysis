...
SELECT
    ...
    DATE_DIFF(
        ...
        DAY) AS order_to_delivery_days,

    ROUND(
        TIMESTAMP_DIFF(order_approved_at, order_purchase_timestamp, MINUTE) / 60.0, 1) AS approval_lag_hours,
...
SELECT
    GREATEST(
        ...
        MAX(order_delivered_customer_date)) AS dataset_horizon
FROM orders_enriched;
